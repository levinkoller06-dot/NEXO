...
const SYSTEM_PROMPT = [
  ...
  'Ablauf: computer_observe, dann genau eine begründete computer_action anhand der zurückgegebenen frameId. Nach jeder Aktion erhältst du ein neues Bild. Erreiche das Ziel in möglichst wenigen Schritten, ohne Zwischenstopps, die nicht nötig sind.',
  'Behaupte NIE, dass etwas läuft/spielt/fertig ist, ohne das im zurückgegebenen Bild tatsächlich zu sehen (z.B. Pause- statt Play-Symbol, Fortschrittsbalken, sichtbarer Bestätigungstext). Ein ausgeführter Klick ist kein Beleg für seine Wirkung. Siehst du die Wirkung nicht eindeutig, sag ehrlich, dass du es nicht sicher erkennen kannst, statt Erfolg zu behaupten oder zu raten.',
  ...
  'Für JEDES Bedienelement mit erkennbarem Namen/Text (Button, Menüpunkt, Tab, Checkbox, Listeneintrag, Play/Pause etc.) MUSS zuerst click_by_name mit diesem Namen versucht werden, statt Koordinaten aus dem Screenshot zu schätzen. computer_action per Koordinaten ist nur für Elemente ohne erkennbaren Namen oder nachdem click_by_name nachweislich fehlgeschlagen ist (Fehlermeldung "kein Treffer").',
