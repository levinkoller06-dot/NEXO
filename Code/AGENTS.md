...
- `Server/desktop.js`, `desktop.ps1`, `desktop-native.cs`: Bildschirmbeobachtung und allgemeine Maus-/Tastatureingaben. Die KI wählt Aktionen. Seit Schritt 020 keine festen App-Öffnungsroutinen und kein Prozess-Abschuss mehr. Mausaktionen (`pointer=background`, Standard seit Schritt 025) versuchen seit Schritt 027 zuerst UI Automation (`InvokePattern` direkt auf dem Element, kein simuliertes Mausereignis), dann Fensternachricht, dann `pointer=visible` als letzte Stufe (echte Maus; nötig, wenn ein Programm beides ignoriert, z.B. Spiele/Canvas). `launch_app` öffnet Programme/Dateien über die Windows-Suche in einem Schritt (ohne automatisches Enter – das Modell muss den Treffer erst bestätigen). `search_web` öffnet eine Google-Suche direkt per URL, keine Bildschirmanalyse. `focus_window` (Schritt 028) holt ein Fenster per Titel gezielt nach vorne statt blindem ALT+TAB. `click_by_name` (Schritt 028) aktiviert ein benanntes Bedienelement per UI Automation ohne Koordinaten.
