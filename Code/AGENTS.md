...
- `Server/desktop.js`, `desktop.ps1`, `desktop-native.cs`: Bildschirmbeobachtung und allgemeine Maus-/Tastatureingaben. Die KI wählt Aktionen. Seit Schritt 020 keine festen App-Öffnungsroutinen und kein Prozess-Abschuss mehr. Mausaktionen laufen seit Schritt 025 standardmäßig als `pointer=background` (Fensternachricht ans Zielfenster, bewegt den sichtbaren Cursor nicht; funktioniert nicht bei jedem Programm, Rückfall `pointer=visible`). `launch_app` öffnet Programme/Dateien über die Windows-Suche in einem Schritt statt mehrerer Beobachtungsrunden.
